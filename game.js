...
(() => {
  ...
  function plantSpriteFile(plant) {
    const map = {
      taodan: "plant_sprite_02.png",
      xiongtongzi: "plant_sprite_03.png",
      yulu: "plant_sprite_03.png",
      nailao: "plant_sprite_08.png",
      zile: "plant_sprite_07.png",
      jingye: "plant_sprite_08.png",
      chengmenglu: "plant_sprite_06.png",
      shengshihua: "plant_sprite_07.png",
      qianchuan: "plant_sprite_10.png",
      xicaishu: "plant_sprite_10.png",
      lanxinghua: "plant_sprite_05.png",
      fenxueshan: "plant_sprite_02.png",
      mansky: "plant_sprite_05.png",
      yangganju: "plant_sprite_05.png",
      xiaoju: "plant_sprite_05.png",
      fenglingcao: "plant_sprite_05.png",
      margaret: "plant_sprite_05.png",
      youhuahunli: "plant_sprite_11.png",
      changshouhua: "plant_sprite_11.png",
      junzilan: "plant_sprite_12.png",
      taiyanghua: "plant_sprite_06.png",
      xunzhangju: "plant_sprite_07.png",
      bohe: "plant_sprite_10.png",
      guibeizhu: "plant_sprite_12.png",
      wenzhu: "plant_sprite_10.png",
      qinyerong: "plant_sprite_06.png",
      maocao: "plant_sprite_01.png",
      maobohe: "plant_sprite_10.png",
      sanhua: "plant_sprite_08.png",
      fenbanxianren: "plant_sprite_04.png",
      xiangrikui: "plant_sprite_05.png",
      liuxingcao: "plant_sprite_09.png"
    };
    return map[plant.id] || `plant_sprite_${String((PLANTS.findIndex((item) => item.id === plant.id) % 12) + 1).padStart(2, "0")}.png`;
  }

  function plantSVG(plant) {
    return `<img class="plant-sprite-img" src="${plantSpriteFile(plant)}" alt="${escapeHTML(plant.name)}">`;
    const stroke = "#4a372c";
    const leaf = plant.color;
    const light = plant.accent;
    const filterId = `rough-${plant.id}`;
    const potColors = {
      hot: "#f3d77b",
      flower: "#bfe1df",
      green: "#f2d477",
    ...
    const pot = potColors[plant.group] || "#f3d77b";
    const defs = `
      <defs>
        <filter id="${filterId}" x="-8%" y="-8%" width="116%" height="116%">
          <feTurbulence type="fractalNoise" baseFrequency="0.82" numOctaves="2" seed="${plant.id.length + plant.name.length}" result="noise"/>
          <feDisplacementMap in="SourceGraphic" in2="noise" scale="0.55"/>
        </filter>
      </defs>`;
    const sparkle = `
      <path d="M19 25 L22 31 L28 34 L22 37 L19 43 L16 37 L10 34 L16 31Z" fill="#f6dc72" stroke="${stroke}" stroke-width="2" stroke-linejoin="round" opacity=".8"/>
      <path d="M96 31 L98 36 L103 38 L98 41 L96 46 L93 41 L88 38 L93 36Z" fill="#f6dc72" stroke="${stroke}" stroke-width="2" stroke-linejoin="round" opacity=".75"/>`;
    const face = `
      <circle cx="51" cy="91" r="2.4" fill="${stroke}"/>
      <circle cx="69" cy="91" r="2.4" fill="${stroke}"/>
      <path d="M56 97 Q60 101 64 97" fill="none" stroke="${stroke}" stroke-width="2.6" stroke-linecap="round"/>
      <circle cx="44" cy="97" r="4.2" fill="#f29a8e" opacity=".72"/>
      <circle cx="76" cy="97" r="4.2" fill="#f29a8e" opacity=".72"/>`;
    const potBase = (extra = "") => `
      <g filter="url(#${filterId})">
        <path d="M34 72 Q60 77 86 72 L80 108 Q60 114 40 108Z" fill="${pot}" stroke="${stroke}" stroke-width="4.4" stroke-linejoin="round"/>
        <path d="M31 70 Q60 62 89 70 Q87 81 60 82 Q33 81 31 70Z" fill="#8b7461" stroke="${stroke}" stroke-width="4.2" stroke-linejoin="round"/>
        ${extra}
        ${face}
      </g>`;
    ...
    if (plant.group === "flower") {
      plantBody = `
        <g filter="url(#${filterId})">
          <path d="M60 75 C58 61 59 49 60 34" stroke="#669766" stroke-width="4.4" fill="none" stroke-linecap="round"/>
          <path d="M59 57 C44 47 35 55 29 64 C43 67 53 63 59 57Z" fill="#8ebf83" stroke="${stroke}" stroke-width="3.2" stroke-linejoin="round"/>
          <path d="M62 54 C76 44 85 51 91 61 C76 65 66 61 62 54Z" fill="#7ead78" stroke="${stroke}" stroke-width="3.2" stroke-linejoin="round"/>
          ${[0, 60, 120, 180, 240, 300].map((deg) => `<ellipse cx="60" cy="20" rx="9.5" ry="18" fill="${leaf}" stroke="${stroke}" stroke-width="3.5" transform="rotate(${deg} 60 35)"/>`).join("")}
          <circle cx="60" cy="35" r="10.5" fill="${light}" stroke="${stroke}" stroke-width="3.2"/>
        </g>
        ${potBase()}`;
    } else if (["fenbanxianren", "shengshihua"].includes(plant.id)) {
      plantBody = `
        <g filter="url(#${filterId})">
          <path d="M44 72 C35 53 40 31 55 31 C65 31 67 45 64 59 C78 49 91 58 85 72" fill="${leaf}" stroke="${stroke}" stroke-width="4.2" stroke-linecap="round" stroke-linejoin="round"/>
          <path d="M55 37 C54 49 55 61 57 72 M72 58 C70 64 70 69 72 73 M43 50 C47 55 49 63 48 72" fill="none" stroke="#5d8e56" stroke-width="2.5" stroke-linecap="round"/>
          <circle cx="54" cy="47" r="2.1" fill="${light}"/><circle cx="43" cy="62" r="2" fill="${light}"/><circle cx="75" cy="66" r="2" fill="${light}"/>
        </g>
        ${potBase()}`;
    } else if (plant.group === "green" || ["maocao", "maobohe"].includes(plant.id)) {
      plantBody = `
        <g filter="url(#${filterId})">
          ${[-35, -20, -5, 12, 29].map((deg, i) => `
            <ellipse cx="${42 + i * 5}" cy="${52 - i * 6}" rx="11" ry="${23 + i % 2 * 5}" fill="${i % 2 ? leaf : light}" stroke="${stroke}" stroke-width="3.4" transform="rotate(${deg} ${42 + i * 5} ${52 - i * 6})"/>
            <path d="M${42 + i * 5} ${69 - i * 5} C${43 + i * 5} ${58 - i * 6} ${43 + i * 5} ${48 - i * 6} ${42 + i * 5} ${35 - i * 3}" fill="none" stroke="#5f8f58" stroke-width="1.9" stroke-linecap="round" opacity=".75"/>
          `).join("")}
        </g>
        ${potBase(plant.id === "taiyanghua" ? `<path d="M53 101 L61 95 L68 101" fill="none" stroke="#e88380" stroke-width="3" stroke-linecap="round"/>` : "")}`;
    } else if (plant.id === "sanhua") {
      plantBody = `
        <g filter="url(#${filterId})">
          <path d="M38 60 C42 36 80 35 84 60 C82 77 41 79 38 60Z" fill="${leaf}" stroke="${stroke}" stroke-width="4.2" stroke-linejoin="round"/>
          <circle cx="50" cy="53" r="8" fill="#fff2d5" stroke="${stroke}" stroke-width="2.5"/>
          <circle cx="70" cy="53" r="8" fill="#2c211b" stroke="${stroke}" stroke-width="2.5"/>
          <circle cx="60" cy="67" r="9" fill="#df9057" stroke="${stroke}" stroke-width="2.5"/>
          <path d="M49 43 Q60 31 72 43" fill="none" stroke="#80b47d" stroke-width="4" stroke-linecap="round"/>
        </g>
        ${potBase()}`;
    } else if (plant.id === "liuxingcao") {
      plantBody = `
        <g filter="url(#${filterId})">
          ${[0, 60, 120, 180, 240, 300].map((deg) => `<ellipse cx="60" cy="38" rx="10" ry="24" fill="${leaf}" stroke="${stroke}" stroke-width="3.5" transform="rotate(${deg} 60 55)"/>`).join("")}
          <path d="M33 23 C50 15 68 12 92 17" fill="none" stroke="#fff0a6" stroke-width="4.2" stroke-linecap="round"/>
          <path d="M85 13 L89 18 L95 19 L90 23 L91 29 L85 26 L80 29 L81 23 L76 19 L82 18Z" fill="#fff0a6" stroke="${stroke}" stroke-width="2.4" stroke-linejoin="round"/>
        </g>
        ${potBase()}`;
    } else {
      plantBody = `
        <g filter="url(#${filterId})">
          ${Array.from({ length: 12 }, (_, i) => {
            const deg = i * 30;
            const rx = i % 2 ? 10 : 13;
            return `<ellipse cx="60" cy="42" rx="${rx}" ry="25" fill="${i % 3 === 0 ? light : leaf}" stroke="${stroke}" stroke-width="3.4" transform="rotate(${deg} 60 58)"/>`;
          }).join("")}
          <circle cx="60" cy="58" r="9" fill="${light}" stroke="${stroke}" stroke-width="2.8"/>
        </g>
        ${potBase(plant.id === "xiongtongzi" ? `<path d="M54 103 Q60 96 66 103" fill="none" stroke="#fff8da" stroke-width="3" stroke-linecap="round"/>` : "")}`;
    }

    return `
      <svg class="doodle-plant doodle-${plant.group}" viewBox="0 0 120 120" aria-hidden="true">
        ${defs}
        <rect x="5" y="5" width="110" height="110" rx="24" fill="rgba(255,248,230,.18)"/>
        ${sparkle}
        ${plantBody}
      </svg>`;
  }

  function catSVG(variant = "main") {
    return `<img class="cat-sprite-img" src="cat_sprite.png" alt="左耳缺角的小三花">`;
    const filterId = `cat-rough-${variant}`;
    return `
      <svg class="doodle-cat" viewBox="0 0 160 160" aria-hidden="true">
        <defs>
          <filter id="${filterId}" x="-8%" y="-8%" width="116%" height="116%">
        ...
        <g filter="url(#${filterId})" stroke="#161210" stroke-width="8.5" stroke-linecap="round" stroke-linejoin="round">
          <path class="cat-tail" d="M106 118 C128 92 153 105 142 134 C134 155 108 151 103 132" fill="#fffaf0"/>
          <path class="cat-body" d="M51 72 C37 87 35 129 56 145 C66 153 95 153 105 145 C126 129 122 87 109 72Z" fill="#fffaf0"/>
          <path class="cat-black-leg" d="M97 112 C110 122 108 143 95 150 L82 150 C89 136 87 122 97 112Z" fill="#161210"/>
          <path class="cat-head" d="M40 73 C32 51 34 25 51 18 L72 42 Q80 37 88 42 L110 18 C128 25 128 51 120 73 C121 99 39 99 40 73Z" fill="#fffaf0"/>
          <path d="M51 19 L59 49 L43 42Z" fill="#161210"/>
          <path d="M110 19 L101 49 L118 42Z" fill="#161210"/>
          <path class="ear-notch" d="M39 22 L33 39 L49 38Z" fill="#f7f1e6" stroke="#f7f1e6"/>
          <path class="calico-dot" d="M67 48 C73 41 82 42 87 49 C79 52 72 52 67 48Z" fill="#e49a58" stroke="none"/>
          <path class="black-spot" d="M104 88 C116 83 127 90 126 104 C115 103 108 98 104 88Z" fill="#161210" stroke="none"/>
          <circle cx="62" cy="74" r="10.5" fill="#161210" stroke="none"/>
          <circle cx="98" cy="74" r="10.5" fill="#161210" stroke="none"/>
          <circle cx="48" cy="91" r="9.5" fill="#f28f84" stroke="none" opacity=".92"/>
          <circle cx="112" cy="91" r="9.5" fill="#f28f84" stroke="none" opacity=".92"/>
          <path d="M78 87 Q80 90 82 87" fill="none" stroke-width="4"/>
          <path d="M80 91 Q75 98 69 94 M80 91 Q85 98 91 94" fill="none" stroke-width="3.6"/>
          <path d="M43 84 C31 80 22 80 14 82 M43 93 C31 94 23 98 15 102 M117 84 C129 80 138 80 146 82 M117 93 C129 94 137 98 145 102" fill="none" stroke-width="4.2"/>
          <path d="M64 149 L64 130 M96 149 L96 130" fill="none" stroke-width="6.5"/>
          <path d="M55 151 C63 154 72 154 80 151 M82 151 C91 154 100 154 107 151" fill="none" stroke-width="3.8"/>
        </g>
      </svg>`;
  }
  ...
  function renderScene() {
    ...
    stage.innerHTML = `
      <div class="visual-${kind}">
        ...
        <div class="shop-cat cat-svg-holder">${catSVG(`scene-${kind}`)}</div>
  ...
  function openSettings() {
    openModal(`
      <h3>设置</h3>
      <p class="story-copy">声音：点击小猫时会播放“喵喵”声；浏览器通常需要首次点击后才允许播放音频。</p>
      <p class="story-copy">存档：游戏进度保存在当前浏览器本地缓存中。清除浏览器数据后，进度和打赏标记可能会重置。</p>
      <p class="story-copy">提示：每日 17:00–20:00 回到结尾自由营业页，可触发限定流星雨。</p>
    `);
  }
  ...
  function renderSide() {
    $("#catButton").className = `cat-button cat-svg-holder ${catStageClass()}`;
    $("#catButton").innerHTML = catSVG("side");
  ...
  function render() {
    const coverCat = $("#coverCat");
    if (coverCat) {
      coverCat.classList.add("cat-svg-holder");
      if (!coverCat.innerHTML.trim()) coverCat.innerHTML = catSVG("cover");
    }
  ...
  function playMeow() {
    try {
      ...
      catAudioContext = catAudioContext || new AudioContextCtor();
      const now = catAudioContext.currentTime;
      const gain = catAudioContext.createGain();
      const osc = catAudioContext.createOscillator();
      const osc2 = catAudioContext.createOscillator();

      osc.type = "triangle";
      osc2.type = "sine";
      osc.frequency.setValueAtTime(620, now);
      osc.frequency.exponentialRampToValueAtTime(430, now + 0.14);
      osc.frequency.exponentialRampToValueAtTime(760, now + 0.32);
      osc2.frequency.setValueAtTime(920, now);
      osc2.frequency.exponentialRampToValueAtTime(680, now + 0.24);
      ...
      console.warn("喵喵声播放失败：", error);
    }
  }

  function showMeowBubble(event) {
    const x = event?.clientX || window.innerWidth / 2;
    const y = event?.clientY || window.innerHeight / 2;
    const bubble = document.createElement("div");
    bubble.className = "meow-bubble";
    bubble.textContent = Math.random() > 0.45 ? "喵～" : "喵喵！";
    bubble.style.left = `${x}px`;
    bubble.style.top = `${y}px`;
    document.body.appendChild(bubble);
    window.setTimeout(() => bubble.remove(), 1300);
  }

  function catMeow(event) {
    playMeow();
    showMeowBubble(event);
  }

  function makeSceneCatRun(event) {
    const cat = event?.target?.closest?.(".shop-cat") || $("#sceneStage .shop-cat");
    if (!cat) return;
    cat.classList.remove("cat-running");
    void cat.offsetWidth;
    cat.classList.add("cat-running");
    window.clearTimeout(cat._runTimer);
    cat._runTimer = window.setTimeout(() => cat.classList.remove("cat-running"), 2600);
  }
  ...
  function bindEvents() {
    ...
    $("#sceneStage").addEventListener("click", (event) => {
      if (!event.target.closest(".shop-cat")) return;
      burstPaws(event);
      catMeow(event);
      makeSceneCatRun(event);
      state.cat.pet += 1;
      state.cat.affection += 1;
      if (state.cat.pet >= 10) {
        unlockEgg("paw10");
